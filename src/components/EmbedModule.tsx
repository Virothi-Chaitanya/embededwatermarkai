...
import { loadImageFromFile, toGrayscale, fromGrayscale, imageDataToDataURL, calculateSSIM, calculateNCC } from "@/utils/imageUtils";
...
export function EmbedModule() {
  ...
  const handleEmbed = useCallback(async () => {
    ...
    try {
      ...
      const embedResult = await runAsync(() => hybridEmbed(coverGray, wmGray));
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <ImageUpload label="Cover Image" description="The host image to embed the watermark into"
        ...
        <ImageUpload label="Watermark Image" description="Binary or grayscale logo/mark to embed"
      ...
      <Button onClick={handleEmbed} disabled={!coverFile || !watermarkFile || processing}
        ...
        {processing ? (
          ...
          <><Lock className="w-4 h-4 mr-2" />Embed Watermark (2-Level DWT + SVD + GA)</>
        )}
      </Button>
